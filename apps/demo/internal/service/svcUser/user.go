...
func (svc *userSvc) Delete(c *gin.Context, req *dtoUser.UserDeleteReq) error {
	deletedBy := contextUtils.GetUserID(c)
...
func (svc *userSvc) PageList(c *gin.Context, req *dtoUser.UserPageListReq) (*dtoUser.UserPageListResp, error) {
	...
	_, _ = storages.RedisClient().Get(c, "test").Result()
	// _, _ = storages.EsClient().Search(
	// 	storages.EsClient().Search.WithContext(c),
	// 	storages.EsClient().Search.WithIndex("accounts"),
	// 	storages.EsClient().Search.WithBody(strings.NewReader(`{"query":{"match_all":{}}}`)),
	// )
